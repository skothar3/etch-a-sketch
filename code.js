...
let adjustedGridDim = gridDim - (2*outerGridLinesDim) - (2*innerGridLinesDim*(numSquaresPerSide-2));
...
bgColorOptions.addEventListener('change',() => {
    ...
    gridDivs.forEach(gridDiv => gridDiv.style.backgroundColor = hue2hsl(bgColorValue));
});
...
gridSlider.addEventListener('change', () => {
    ...
    adjustedGridDim = gridDim - (2*outerGridLinesDim) - (2*innerGridLinesDim*(numSquaresPerSide-2));
    createGrid();
})
...
function shading(rgb,amount) {
    ...
    let hslNew;
    if((amount < 0 && (hsl[2] + amount >= 0)) || (amount > 0 && (hsl[2] + amount <= 100))){
...
function rgb2hsl(rgb) {
    const rgbArray = rgb.slice(rgb.indexOf('(')+1,rgb.indexOf(')')).split(','). map(num => parseFloat(num));
    ...
    if (c == 0) {
      ...
      switch(max) {
        case r:
          ...
          if (segment < 0) {          // H > 180, full rotation
  ...
  function createGrid() {
    ...
    mouseIsDown = false;
    gridDivs.forEach(gridDiv => gridDiv.addEventListener('mousedown',(e) => {
        mouseIsDown = true;
        draw(e);
        }));
    gridDivs.forEach(gridDiv => gridDiv.addEventListener('mouseup',() => {mouseIsDown = false;}));
